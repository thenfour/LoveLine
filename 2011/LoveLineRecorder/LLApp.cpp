...
#define CAPTURE_INTERVAL (1)// seconds
...
void ScreenToClientRect(HWND hWnd, RECT* prc)
{
    POINT p = {prc->left, prc->top};
    POINT p2 = {prc->right, prc->bottom };
    ScreenToClient(hWnd, &p);
    ScreenToClient(hWnd, &p2);
    SetRect(prc, p.x, p.y, p2.x, p2.y);
}


BOOL IsValidBitRate(DWORD dw)
{
    switch(dw)
    {
    case 16:
...
LLMainInitVars::~LLMainInitVars()
{
    ...
    if(this->hTitle) DeleteObject(this->hTitle);
    CloseHandle(this->hEncodingCancel);
}
...
bool LLApp::LM_SetExtents(long lmin, long lmax)
{
    PostMessage(m_lv, LVM_SETRANGE, (WPARAM)lmin, (WPARAM)lmax);
    return true;
}


bool LLApp::LM_SetValue(long pos)
{
    PostMessage(m_lv, LVM_SETPOS, (WPARAM)0, (WPARAM)pos);
    return true;
}
...
/*
    ...
    -V "{...} or name"    Device to use (can be GUID, module name, or description.)
    ...
    -Q Bitrate            Bit rate for encoded output (KBPS). (default 32)
    -A                    Add output file to default database (default YES)
...
int LLApp::Entry(HINSTANCE hInstance, PCWSTR wszCmdLine)
{
        ...
        if(wcslen(wszArg) == 2)
        {
            if(wszArg[0] == '/' || wszArg[0] == '-')
            {
                ...
                case 'q':
                    iArg++;
                    n = wcstol(argv[iArg], 0, 10);
                    if(IsValidBitRate(n)) Vars.nBitRate = n;
                    break;
                case 'S':
                case 's':
                    iArg++;
                ...
                case '?':
                    MessageBox(0,
                        ...
                        L"-V \"{...} or name\"    Device to use (can be GUID, module name, or description.)\n"
                        ...
                        L"-Q Bitrate            Bit rate for encoded output (Kbps). (default 32)\n"
                        L"-A                    Add output to LoveLine database (default YES)\n"
                        L"-?                    Display help\n",
                        L"LoveLine Recorder",
                        MB_OK);
    ...
    GetLocalTime(&this->st);
    GetModuleFileName(hInstance, wszThisModule, MAX_PATH);

    swprintf(wszTime, L"..\\Media\\%04d-%02d-%02d(%02d-%02d-%02d)." DEFEXT,
        this->st.wYear, this->st.wMonth, this->st.wDay,
        this->st.wHour, this->st.wMinute, this->st.wSecond);
    ...
    */
	...
	psp.pszTemplate =		MAKEINTRESOURCE(IDD_DEVICE);
    psp.pfnDlgProc =		LLApp::DeviceProc;
    ...
    */
    ...
    psh.hwndParent =     NULL;
    psh.phpage =         hPages;
    psh.hIcon = LoadIcon(this->hInstance, MAKEINTRESOURCE(IDI_ICON1));
...
INT_PTR CALLBACK LLApp::PropSheetSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NCLBUTTONDOWN:
        {
            ...
            */
            ...
            {
                SetCapture(hWnd);// After we start capturing, our messages will just be WM_MOUSEMOVE.
                pVars->pThis->bIsTracking = TRUE;
                return 0;
            }
            break;
        }
    case WM_LBUTTONUP:
        {
            // Do a non-client hit test
            if(pVars->pThis->bIsTracking == TRUE)
            {
                pVars->pThis->bIsTracking = FALSE;
                ReleaseCapture();
                ...
                if(SendMessage(hWnd, WM_NCHITTEST, 0, lp) == HTMINBUTTON)
                {
    ...
    return pVars->pThis->pOldProc(hWnd, uMsg, wParam, lParam);
}
	...
	case WM_INITDIALOG:
		{
            ...
            dwStyle = GetWindowLong(hPropSheet, GWL_STYLE) | WS_MINIMIZEBOX;
            SetWindowLong(hPropSheet, GWL_STYLE, dwStyle);

            // Now subclass it so we can accept minimize operations.
            SetWindowLongPtr(hPropSheet, GWL_USERDATA, (LONG_PTR)pVars);
            pVars->pThis->pOldProc = (DLGPROC)GetWindowLongPtr(hPropSheet, GWL_WNDPROC);
            SetWindowLongPtr(hPropSheet, GWL_WNDPROC, (LONG_PTR)LLApp::PropSheetSubclassProc);
...
BOOL CALLBACK LLApp::DSEnumCallback(LPGUID lpGuid, LPCWSTR lpcstrDescription, LPCWSTR lpcstrModule, LPVOID lpContext)
{
    LLApp* pThis = (LLApp*)lpContext;
    LLRecorderDevice dev;

    dev.m_sDescription = lpcstrDescription;
    dev.m_sModule = lpcstrModule;

    if(!lpGuid)
    {
        GUID idDefaultCapture =
            { 0xdef00001, 0x9c6d, 0x47ed,
                { 0xaa, 0xf1, 0x4d, 0xda, 0x8f, 0x2b, 0x5c, 0x03 }
            };

        dev.m_sGuid.FromGuid(&idDefaultCapture);
    }
    else
    {
        dev.m_sGuid.FromGuid(lpGuid);
    }
    
    // insert it into the list.
    pThis->m_Devices.insert(DeviceList_T::value_type((PCWSTR)dev.m_sDescription, dev));
...
// IDC_DEVICE
// IDC_EXTENDEDINFO
INT_PTR CALLBACK LLApp::DeviceProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    LLMainInitVars* pVars = (LLMainInitVars*)GetWindowLongPtr(hWnd, GWL_USERDATA);
                    ...
                    case CBN_SELCHANGE:
                        {
                            // get extended info.
                            CCString s;
                            s.FromWindow(GetDlgItem(hWnd, IDC_DEVICE));

                            LLRecorderDevice* pDev;
                            DeviceList_T::iterator it;
                            it = pVars->pThis->m_Devices.find((PCWSTR)s);
                            if(it != pVars->pThis->m_Devices.end())
                            {
                                pDev = &(it->second);
                                CCString sExt;
                                sExt.FormatW(L"Description:\r\n%s\r\n\r\n"
                                    L"GUID:\r\n%s\r\n\r\n"
                                    L"Module:\r\n%s\r\n\r\n", (PCWSTR)pDev->m_sDescription, (PCWSTR)pDev->m_sGuid, (PCWSTR)pDev->m_sModule);
                                SetDlgItemText(hWnd, IDC_EXTENDEDINFO, sExt);
                            }
                            return TRUE;
                        }
                    }
	...
	case WM_INITDIALOG:
		{ 
            ...
            DirectSoundCaptureEnumerate(LLApp::DSEnumCallback, pVars->pThis);

            // now populate the combobox
            DeviceList_T::iterator it;
            for(it=pVars->pThis->m_Devices.begin();it!=pVars->pThis->m_Devices.end();it++)
            {
                SendDlgItemMessage(hWnd, IDC_DEVICE, CB_INSERTSTRING, -1, (LPARAM)((PCWSTR)it->second.m_sDescription));
            }
            ...
            GUID idDevice;
            CCString descDevice;
            t.GetMatchingDevice(pVars->Device, idDevice, descDevice);

            SendDlgItemMessage(hWnd, IDC_DEVICE, CB_SELECTSTRING , -1, (LPARAM)((PCWSTR)descDevice));
            SendMessage(hWnd, WM_COMMAND, MAKEWPARAM(IDC_DEVICE, CBN_SELCHANGE), 0);
            ...
            case PSN_KILLACTIVE:
                {
                    ...
                    */
                    ...
                    if(it != pVars->pThis->m_Devices.end())
                    {
                        ...
                        // we have the device.
                        pVars->Device = pDev->m_sGuid;
                        pVars->pThis->m_sDeviceDesc = pDev->m_sDescription;
                    }
		    ...
		    case PSN_SETACTIVE: //Enable the Next button
                {
                    ...
                    swprintf(wsz, 
                        ...
                        L"%s\r\n",
                        (PCWSTR)pVars->pThis->m_sDeviceDesc,
                        (PCWSTR)pVars->Device,
                        fkhz,
...
#define ADDBITRATE(a) \
            ...
            CheckDlgButton(hWnd, IDC_ADDTODB, pVars->bRegisterInDB == TRUE ? BST_CHECKED : BST_UNCHECKED);
            ...
            case PSN_KILLACTIVE:
                {
                    ...
                    */
                    ...
                    if(IsValidBitRate(n) == FALSE)
                    {
                        MessageBox(hWnd, L"The bit rate is out of range",
                            L"LoveLine Recorder", MB_OK | MB_ICONASTERISK);
                        SetWindowLong(hWnd, DWL_MSGRESULT, TRUE);
                        return TRUE;
                    }
                    ...
                    // Get Addtodb stuff.
                    pVars->bRegisterInDB = IsDlgButtonChecked(hWnd, IDC_ADDTODB);
	...
	case WM_INITDIALOG:
		{
            ...
            // Replace the meter static window with a REAL level meter
            HWND hMeter = GetDlgItem(hWnd, IDC_METER);
            RECT orect, prect;
            GetWindowRect(hMeter, &orect);
            SetRect(&prect, 0, 0, 0, 0);
            ScreenToClientRect(hWnd, &orect);
            DestroyWindow(hMeter);

            pVars->pThis->m_lv.Create(hWnd, prect, orect);
            ...
            case PSN_QUERYCANCEL:
                {
                    ...
                    int n = MessageBox(hWnd, L"Are you sure you want to cancel and exit?",
                        L"LoveLine Recorder", MB_YESNO);
                    SetWindowLong(hWnd, DWL_MSGRESULT, n == IDYES ? FALSE : TRUE);
                    return TRUE;
                }
            case PSN_RESET:
		    	//Handle a Cancel button click, if necessary
...
HRESULT LLApp::StartRecording(LLMainInitVars* pVars)
{
    ...
    if(FAILED(this->Capturer.Initialize(pVars->Device, &Format, LLApp::CaptureDataProc, pVars, CAPTURE_INTERVAL, &pVars->Log)))
    {
        pVars->Log.Refresh();
        return E_FAIL;
    }

    this->m_lm.Create(this, Format);
...
HRESULT CALLBACK LLApp::CaptureDataProc(PBYTE pData, DWORD dwSize, PVOID pUser)
{
    ...
    pThis->m_lm.NewData(pData, dwSize);
...
UINT _stdcall LLApp::EncodeThreadProc(PVOID pUser)
{
    CoInitialize(NULL);
    ...
    while(1)
    {
        ...
        if(FAILED(hr) || (br != dwBufSize))
        {
            ...
            */
            if(pVars->bRegisterInDB == TRUE)
            {
                pVars->pThis->AddToDatabase(pVars);
            }
...
HRESULT LLApp::AddToDatabase(LLMainInitVars* pVars)
{
    LLDatabase db;
    LLEpisode e;
    HRESULT hr;
    ...
    pVars->Log.AddEntry(L"Opening the database");

    // Construct the database path.
    GetModuleFileName(GetModuleHandle(NULL), DbName.GetBuffer(MAX_PATH), MAX_PATH);
    MediaPath = DbName;
    FileName = pVars->FileName;

    // We go two levels up because the name of the file is counted as a folder in this function.
    DbName.Path_Append(L"..\\..\\Database\\db.xml");

    if(FAILED(hr = db.Open(DbName, &pVars->Log)))
    {
        pVars->Log.AddEntry(L"Could not open the database");
        return E_FAIL;
    }

    MediaPath.Path_Append(L"..\\..\\Media");
    FileName.Path_AbsoluteToRelative(MediaPath);

    e.Description = L"";
    e.FileName = FileName;
    e.Guests = L"";
    e.dwDay = this->st.wDay;
    e.dwMonth = this->st.wMonth;
    e.dwYear = this->st.wYear;

    if(FAILED(db.AddEpisode(&e, &pVars->Log)))
    {
        pVars->Log.AddEntry(L"Could not add the entry to the database");
        return E_FAIL;
    }
...
HRESULT LLApp::StartEncoding(LLMainInitVars* pVars)
{
    ...
    UINT dwThreadId;
	pVars->hEncodingThread = (HANDLE)_beginthreadex(
        0, 0, LLApp::EncodeThreadProc, pVars, 0, &dwThreadId);
	...
	case WM_NOTIFY:
		{
            ...
            case PSN_QUERYCANCEL:
                {
                    // Ask if they really want to quit recording or not
                    int n = MessageBox(hWnd, L"Are you sure you want to stop encoding?",
                        L"LoveLine Recorder", MB_YESNO);
                    SetWindowLong(hWnd, DWL_MSGRESULT, n == IDYES ? FALSE : TRUE);
                    return TRUE;
                }
