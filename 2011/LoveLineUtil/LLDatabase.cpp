...
void EncodeDBString(CCString* p)
{
    p->Encode(L'%', L"&<>\"%");
}


void DecodeDBString(CCString* p)
{
    p->Decode(L'%');
}
...
HRESULT LLDatabase::Open(CCString FileName, CCLog* pErrText)
{
	...
	/*
		Duplicate instance prevention first.
		We create a mutex name like this:
		if the filename is c:\\loveline\\Database\\db.xml
		c0ae86465bff4affbec11037f3c30a85lmx.dbesabatadenilevolc

		Where path is the name of the database, backwards, removing '\\' and ':', 
		and the whole string is truncated at 200 characters.
		
	*/
	CCString Mutex = L"c0ae86465bff4affbec11037f3c30a85";
	CCString MutexFileName = FileName;
	MutexFileName.RemoveAll(L'\\');
	MutexFileName.RemoveAll(L':');
	MutexFileName.rev();
	Mutex += MutexFileName;
	Mutex.Truncate(200);

	// First try to open the mutex.  If it succeeds, we're in bad shape.
	this->hMutexDb = OpenMutex(SYNCHRONIZE, FALSE, Mutex);
	if(this->hMutexDb != NULL && this->hMutexDb != INVALID_HANDLE_VALUE)
	{
		CloseHandle(this->hMutexDb);
		pErrText->AddEntry(L"The database could not be opened because it's currently open by another application.");
		return E_FAIL;
	}

	this->hMutexDb = CreateMutex(NULL, TRUE, Mutex);

    /*
        We determine if the file already exists.  If it does, use it.
    ...
    */
	...
	{
        pErrText->AddEntry(L"Could not load the XML file.");
		IXMLDOMParseError* pParse = NULL;
		BSTR bstr;

		if(SUCCEEDED(this->pDoc->get_parseError(&pParse)))
		{
			long l;
			pParse->get_errorCode(&l);
			pErrText->AddEntry(L"ErrCode: %d", l);
			pParse->get_filepos(&l);
			pErrText->AddEntry(L"FilePosition: %d", l);
			pParse->get_line(&l);
			pErrText->AddEntry(L"Line: %d", l);
			pParse->get_linepos(&l);
			pErrText->AddEntry(L"LinePos: %d", l);
			pParse->get_reason(&bstr);
			pErrText->AddEntry(L"Reason: %s", bstr);
			SysFreeString(bstr);
		}
...
HRESULT LLDatabase::Save(CCLog* pLog)
{
    HRESULT hr;

    if(this->pDoc)
    {
        if(FAILED(hr = this->pDoc->save(this->FileName)))
        {
            pLog->AddEntry(hr, L"IXMLDOMDocument::Save()");
            return E_FAIL;
        }
    }
    return S_OK;
...
HRESULT LLDatabase::_AddEpisode(LLEpisode* pEpisode, CCLog* pErrText)
{
    ...
    */
    ...
    {
        pErrText->AddEntry(L"XML AddEpisode: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::_AddCommercial(LLCommercial* pCommercial, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML _AddCommercial: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::_AddFeature(LLFeature* pFeature, CCLog* pErrText)
{
    ...
    */
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML _AddFeature: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::_AddLink(LLLink* pLink, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML _AddLink: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::EnumCommercials(GUID& Episode, ENUMCOMMERCIALSPROC pProc, PVOID pUser, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML EnumCommercials: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::EnumEpisodes(ENUMEPISODESPROC pProc, PVOID pUser, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML EnumEpisodes: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::EnumFeatures(ENUMFEATURESPROC pProc, PVOID pUser, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML EnumFeatures: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::EnumLinks(ENUMLINKSPROC pProc, PVOID pUser, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML EnumLinks: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::EnumEpisodes(GUID& Feature, ENUMEPISODESPROC2 pProc, PVOID pUser, CCLog* pLog)
{
    ...
    if(this->pDatabase == NULL)
    {
        pLog->AddEntry(L"XML EnumEpisodes: The application has tried to perform operations on an uninitialized object.");
        //return E_FAIL;
    }
...
IXMLDOMNode* LLDatabase::_GetLink(GUID& Link, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML _GetLink: The application has tried to perform operations on an uninitialized object.");
        return NULL;
    }
...
IXMLDOMNode* LLDatabase::_GetCommercial(GUID& Commercial, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML _GetCommercial: The application has tried to perform operations on an uninitialized object.");
        return NULL;
    }
...
IXMLDOMNode* LLDatabase::_GetFeature(GUID& Feature, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML _GetFeature: The application has tried to perform operations on an uninitialized object.");
        return NULL;
    }
...
IXMLDOMNode* LLDatabase::_GetEpisode(GUID& Episode, CCLog* pErrText)
{
    ...
    if(this->pDatabase == NULL)
    {
        pErrText->AddEntry(L"XML _GetEpisode: The application has tried to perform operations on an uninitialized object.");
        return NULL;
    }
...
HRESULT LLDatabase::_AppendChild(IXMLDOMNode* pNewParent, IXMLDOMNode* pNewChild, CCLog* pErrText)
{
    ...
    {
        pErrText->AddEntry(L"XML _AppendChild: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::_AppendText(IXMLDOMNode* pNode, CCString Text, CCLog* pErrText)
{
    ...
    {
        pErrText->AddEntry(L"XML _AppendText: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::SetEpisode(LLEpisode* pEpisode, CCLog* pLog)
{
    ...
    */
    ...
    s = pEpisode->Description;
    EncodeDBString(&s);
    if(FAILED(this->_SetAttribute(pElement, L"Description", s, pLog)))
    {
    ...
    s = pEpisode->Guests;
    EncodeDBString(&s);
    if(FAILED(this->_SetAttribute(pElement, L"Guests", s, pLog)))
    {
    ...
    s = pEpisode->FileName;
    EncodeDBString(&s);
    if(FAILED(this->_SetAttribute(pElement, L"FileName", s, pLog)))
    {
...
{
  IXMLDOMNode* node = _GetFeature(pFeature->guid, pLog);
  if(!node)
  {
    return E_FAIL;
  }

  // set the attributes Text, and Title
  IXMLDOMElement* el = 0;
  if(FAILED(node->QueryInterface(IID_IXMLDOMElement, (void**)&el)))
  {
    pLog->AddEntry("QueryInterface for IID_IXMLDOMElement failed.");
    SAFE_RELEASE(node);
    return E_FAIL;
  }
  SAFE_RELEASE(node);

  if(FAILED(el->setAttribute(CCString("Title").bstr(), pFeature->Title.variant())))
  {
    pLog->AddEntry("Error setting the Title attribute.");
    SAFE_RELEASE(el);
    return E_FAIL;
  }

  if(FAILED(el->setAttribute(CCString("Text").bstr(), pFeature->Text.variant())))
  {
    pLog->AddEntry("Error setting the Text attribute.");
    SAFE_RELEASE(el);
    return E_FAIL;
  }
...
HRESULT LLDatabase::GetSetting(CCString Name, CCString* pValue, CCLog* pLog)
{
    ...
    if(this->pDatabase == NULL)
    {
        pLog->AddEntry(L"XML GetSetting: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::SetSetting(CCString Name, CCString Value, CCLog* pLog)
{
    ...
    if(this->pDatabase == NULL)
    {
        pLog->AddEntry(L"XML SetSetting: The application has tried to perform operations on an uninitialized object.");
        return E_FAIL;
    }
    ...
    s = Name;
    EncodeDBString(&s);
    if(FAILED(hr = pElement->setAttribute((BSTR)XPath, (VARIANT)s)))
    {
...
HRESULT LLDatabase::SetSegmentData(int nSegments, LLSegment* pSegments, CCLog* pLog)
{
//    HRESULT hr;
    CCString s;

    s.FromInt(nSegments, 10);
    if(FAILED(this->SetSetting(L"SEGMENTCOUNT", s, pLog)))
    {
        pLog->AddEntry(L"Error writing the SEGMENTCOUNT setting.");
        return E_FAIL;
    }

    s.FromBinary(pSegments, nSegments * sizeof(LLSegment));
    if(FAILED(this->SetSetting(L"SEGMENTDATA", s, pLog)))
    {
        pLog->AddEntry(L"Error writing the SEGMENTDATA setting.");
        return E_FAIL;
    }
...
HRESULT LLDatabase::GetSegmentCount(int* piOut, CCLog* pLog)
{
    CCString s;

    if(piOut) *piOut = 0;

    if(FAILED(this->GetSetting(L"SEGMENTCOUNT", &s, pLog)))
    {
        // The setting doens't exist.  Let's use default values
        // and write it into the database.
        if(FAILED(this->_SetDefaultSegmentData(pLog)))
        {
            pLog->AddEntry(L"Could not work with segment data.");
            return E_FAIL;
        }

        if(FAILED(this->GetSetting(L"SEGMENTCOUNT", &s, pLog)))
        {
            pLog->AddEntry(L"Second attempt to get the segment count failed.  Segment data could not be read.");
            return E_FAIL;
        }
    }

    if(piOut) *piOut = s.ToInt(10);

    return S_OK;
}


HRESULT LLDatabase::GetSegmentData(LLSegment* pSegments, CCLog* pLog)
{
    CCString s;
    int nSegments;

    if(FAILED(this->GetSegmentCount(&nSegments, pLog)))
    {
        pLog->AddEntry(L"Could not read segment count, so reading segment data failed.");
        return E_FAIL;
    }

    if(FAILED(this->GetSetting(L"SEGMENTDATA", &s, pLog)))
    {
        pLog->AddEntry(L"Could not read segment data.");
        return E_FAIL;
    }

    s.ToBinary(pSegments, sizeof(LLSegment) * nSegments);

    return S_OK;
}


HRESULT LLDatabase::_SetDefaultSegmentData(CCLog* pLog)
{
    CCString s;
    LLSegment Segment;
    ...
    if(FAILED(this->SetSetting(L"SEGMENTCOUNT", s, pLog)))
    {
        pLog->AddEntry(L"Could not set the SEGMENTCOUNT setting.  Segment data could not be saved.");
        return E_FAIL;
    }

    Segment.dwIn = 0;
    Segment.dwOut = INFINITE;
    Segment.dwSize = (27 + (4 * 60)) * 1000;// 4 minutes, 27 seconds.
    s.FromBinary(&Segment, sizeof(Segment));

    if(FAILED(this->SetSetting(L"SEGMENTDATA", s, pLog)))
    {
        pLog->AddEntry(L"Could not set the SEGMENTDATA setting.  Segment data could not be saved.");
        return E_FAIL;
    }
