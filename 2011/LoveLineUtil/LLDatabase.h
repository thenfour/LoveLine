...
/*
    This is used for defining the segment information.
    The data is stored in the following settings:
    SEGMENTCOUNT
    SEGMENTDATA
*/
class LLSegment
...
public:
    ...
    LLCommercial(const LLCommercial& x)
    {
        memcpy(&guid, &x.guid, sizeof(GUID));
        memcpy(&guidEpisode, &x.guidEpisode, sizeof(GUID));
        dwIn = x.dwIn;
        dwOut = x.dwOut;
    }
...
void CCEXPORT EncodeDBString(CCString* p);
void CCEXPORT DecodeDBString(CCString* p);
...
public:
    ...
    HRESULT Close();
    HRESULT Save(CCLog* pLog);

    /*
        For segment/commercial data.
    */
    HRESULT SetSegmentData(int nSegments, LLSegment* pSegments, CCLog* pLog);
    HRESULT GetSegmentCount(int* piOut, CCLog* pLog);
    HRESULT GetSegmentData(LLSegment* pSegments, CCLog* pLog);
    ...
    */
    ...
    //HRESULT EnumLinks(GUID& feature, std::vector<LLLink>& result, CCLog* pErrText);
...
private:
    ...
    HRESULT _SetDefaultSegmentData(CCLog* pLog);
	...
	HANDLE hMutexDb;// Duplicate instance prevention.
