...
// 030101 Carl Corcoran
void CCString::cat(WCHAR ch)
{
    WCHAR s[2] = { ch, 0 };
    this->_CatW(s);
    return;
...
void CCString::Path_RenameExtension(PCWSTR wsz)
{
    _SizeSetPreserve(MAX_PATH);
    PathRenameExtension(this->wszString, wsz);
}
...
{
    WCHAR wsz[50] = {0};
    PWSTR wszFrom = wszString;
    if(wszFrom[0] == '{') wszFrom++;
    wcsncpy(wsz, wszFrom, 36);
    return UuidFromStringW((RPC_WSTR)wsz, pGuid);
}
...
HRESULT CCString::FromWindow(HWND hWnd)
{
    ...
    GetWindowText(hWnd, this->GetBuffer(n + 1), n + 1);
...
*/
HRESULT CCString::FromBinary(PVOID p, int nSizeBytes)
{
    PBYTE pData = (PBYTE)p;
    int i;
    PWSTR wsz;
