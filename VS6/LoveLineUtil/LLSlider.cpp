...
#define SHIFT_SPEED_DELTA 2000
#define X_SIZE 10
#define DELETE_TOLERANCE 100
...
LLSlider::~LLSlider()
{
    ...
    SAFE_DELETEOBJECT(this->hBookmarkHandle);
    SAFE_DELETEOBJECT(this->hCommercialSelected);
    SAFE_DELETEOBJECT(this->hCommercialHandle);
    SAFE_DELETEOBJECT(this->hFlatLine);
    SAFE_DELETEOBJECT(this->hCursor);
    SAFE_DELETEOBJECT(this->hHighlight);
    SAFE_DELETEOBJECT(this->hCommercial);
    SAFE_DELETEOBJECT(this->hBackground);
    SAFE_DELETEOBJECT(this->hBookmark);
    SAFE_DELETEOBJECT(this->hDelete);
...
DWORD LLSlider::TimeToPixels(DWORD dwMS)
{
    ...
    dw = (DWORD)(((__int64)nWidth * (__int64)dwMS) / (__int64)this->dwLength);
...
DWORD LLSlider::PixelsToTime(int dwPixels)
{
    ...
    dw = (DWORD)(((__int64)dwPixels * (__int64)this->dwLength) / (__int64)nWidth);
            ...
            else
            {
                ...
                if(pThis->pdw)
                {
                    ...
                    */
                    if(wParam & MK_SHIFT)
                    {
                        // We only let the mouse move one pixel here.
                        int nMouseChange = p.x - nOldx;

                        if(nMouseChange > 0)
                        {
                            dwNewPos = (*pThis->pdw) + SHIFT_SPEED_DELTA;
                            if(dwNewPos > pThis->dwLength) dwNewPos = pThis->dwLength;
                        }
                        else
                        {
                            if((*pThis->pdw) > SHIFT_SPEED_DELTA)
                            {
                                dwNewPos = (*pThis->pdw) - SHIFT_SPEED_DELTA;
                            }
                            else
                    ...
                    *pThis->pdw = dwNewPos;
                    ...
                    // to delete the item...
                    if(p.y < (pThis->rcWindow.top - DELETE_TOLERANCE) || 
						p.y > (pThis->rcWindow.bottom + DELETE_TOLERANCE))
                    {
