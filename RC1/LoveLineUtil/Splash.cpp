...
DWORD WINAPI CCSplash::ThreadProc(PVOID p)
{
    CCSplash* pThis = (CCSplash*)p;
	WNDCLASSW wc;
    pThis->sStaticText = L"Initializing Splash Screen";
    ...
    pThis->hWnd = CreateWindowExW(
        0,//WS_EX_TOPMOST, 
        pThis->sClass, L"", pThis->dwStyle,
		0, 0, 1, 1,
		(HWND)pThis->hParent,
    ...
    UnregisterClassW(pThis->sClass, pThis->hInstance);
    pThis->hWnd = NULL;
